        ...
        for categ in self.emp_id.category_ids:
            if categ.salary_advance_policy:
                for p in categ.salary_advance_policy:
                    ...
                    if ds.day <= int(p.day):
                        raise UserError(_('Warning !'), _('You can not take advance in current date as policy does not allow you to take advance before specific day.'))
                    if p.method == 'fixed' and p.policy_value < self.advance:
                        raise UserError(_('Warning !'), _('You are exceeding the limit of advance as salary advance policy set for : %s not allowed.') % (_(self.emp_id.name)))
                    if p.method == 'basic':
...
#            partner_id = line.asset_id.partner_id.id
            ...
            if not address_id:
                raise UserError( _('There is no home address defined for employee: %s ') % (_(line.emp_id.name)))
            partner_id = address_id and address_id.id or False
            if not partner_id:
                raise UserError( _('There is no partner defined for employee : %s ') % (_(line.emp_id.name)))
            sign = -1
...
#            self.write(cr, uid, line.id, {'move_id': move_id}, context=context)
    ...
    journal_id = fields.Many2one('account.journal', 'Journal', default=_get_journal)
    ...
    employee_account = fields.Many2one('account.account', relation='account.account', string="Employee Account", domain="[('account_type','=','receivable')]", default=_get_account)
