...
class AccountPayment(models.Model):
    ...
    destination_account_id = fields.Many2one(
        ...
        domain="[('account_type', 'in', ('asset_receivable', 'other', 'payable')), ('company_id', '=', company_id)]",
        check_company=True,
    )

    @api.model
    def default_get(self, vals):
        res = super(AccountPayment, self).default_get(vals)
        settings = self.env["res.config.settings"].sudo().get_values()
        res.update(
            {
                "advance_account_id": settings.get("default_advance_account_id"),
            }
        )
        return res
    ...
    def _synchronize_from_moves(self, changed_fields):
        ...
        for pay in self.with_context(skip_account_move_synchronization=True):
            ...
            if "line_ids" in changed_fields:
                ...
                if len(counterpart_lines) != 1 and not pay.is_advance:
                    raise UserError(
                        _(
                            "Journal Entry %s is not valid. In order to proceed, the journal items must "
                            "include one and only one asset_receivable/payable account (with an exception of "
                            "internal transfers).",
                            move.display_name,
                        )
                    )
    ...
    def _compute_destination_account_id(self):
        ...
        for pay in self:
            if pay:
                ...
                if pay.partner_id:
                    if pay.is_advance:
                        pay.destination_account_id = self.advance_account_id
                    else:
