...
class TestPaymentTransaction(MonnifyCommon):

    def test_no_item_missing_from_rendering_values(self):
        """ Test that the rendered values are conform to the transaction fields. """
        tx = self._create_transaction(flow='redirect')
        with patch(
            'odoo.addons.payment_monnify.models.payment_provider.PaymentProvider'
            '._monnify_make_request', return_value={'data': {'link': 'https://dummy.com'}}
        ):
    ...
    def test_no_input_missing_from_redirect_form(self):
        ...
        tx = self._create_transaction(flow='redirect')
        with patch(
            'odoo.addons.payment_monnify.models.payment_transaction.PaymentTransaction'
    ...
    def test_processing_notification_data_confirms_transaction(self):
        ...
        with patch(
            'odoo.addons.payment_monnify.models.payment_provider.PaymentProvider'
            '._monnify_make_request', return_value=self.verification_data
        ):
    ...
    def test_processing_notification_data_tokenizes_transaction(self):
        ...
        with patch(
            ...
            'odoo.addons.payment_monnify.models.payment_transaction.PaymentTransaction'
            '._monnify_tokenize_from_notification_data'
        ) as tokenize_mock:
            tx._process_notification_data(self.redirect_notification_data)
